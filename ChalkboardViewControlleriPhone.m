//
//  ChalkboardViewControlleriPhone.m
//  LearnToWrite
//
...
//Changed for v4.0
//These are imported into the .h file already because this class is a delegate of these other classes.
//#import "ChalkboardViewiPhone.h"
//#import "AlphabetViewController.h"
#import "SoundEffect.h"
#import "AAnimationViewControlleriPhone.h"
#import "BAnimationViewControlleriPhone.h"
#import "CAnimationViewControlleriPhone.h"
#import "DAnimationViewControlleriPhone.h"
#import "EAnimationViewControlleriPhone.h"
#import "FAnimationViewControlleriPhone.h"
#import "GAnimationViewControlleriPhone.h"
#import "HAnimationViewControlleriPhone.h"
#import "IAnimationViewControlleriPhone.h"
#import "JAnimationViewControlleriPhone.h"
#import "KAnimationViewControlleriPhone.h"
#import "LAnimationViewControlleriPhone.h"
#import "MAnimationViewControlleriPhone.h"
#import "NAnimationViewControlleriPhone.h"
#import "OAnimationViewControlleriPhone.h"
#import "PAnimationViewControlleriPhone.h"
#import "QAnimationViewControlleriPhone.h"
#import "RAnimationViewControlleriPhone.h"
#import "SAnimationViewControlleriPhone.h"
#import "TAnimationViewControlleriPhone.h"
#import "UAnimationViewControlleriPhone.h"
#import "VAnimationViewControlleriPhone.h"
#import "WAnimationViewControlleriPhone.h"
#import "XAnimationViewControlleriPhone.h"
#import "YAnimationViewControlleriPhone.h"
#import "ZAnimationViewControlleriPhone.h"
#import "ZeroAnimationViewControlleriPhone.h"
#import "OneAnimationViewControlleriPhone.h"
#import "TwoAnimationViewControlleriPhone.h"
#import "ThreeAnimationViewControlleriPhone.h"
#import "FourAnimationViewControlleriPhone.h"
#import "FiveAnimationViewControlleriPhone.h"
#import "SixAnimationViewControlleriPhone.h"
#import "SevenAnimationViewControlleriPhone.h"
#import "EightAnimationViewControlleriPhone.h"
#import "NineAnimationViewControlleriPhone.h"
#import "TenAnimationViewControlleriPhone.h"
...
@implementation ChalkboardViewControlleriPhone

@synthesize animationSpeediPhone;
@synthesize scrollViewiPhone;
@synthesize alphabetViewController;
@synthesize chalkboardViewiPhone;
@synthesize letterLabeliPhone;
@synthesize guidlinesImage;
@synthesize choosPaintbrushButton;
...
@synthesize cancelGuessGameButtoniPhone;
@synthesize replayGuessAnimationButtoniPhone;
...
@synthesize guessingGameButtoniPhone;
@synthesize eraseButtoniPhone;
@synthesize settingsMenuButtoniPhone;
...
//Also need to play an audio file when the guess game is cancelled that says something like "Try again later".
...
#pragma mark - When the View Loads
 ...
 - (void)viewWillAppear:(BOOL)animated {
     ...
     //Need this to set the width of the view correctly based on whether it's showing numbers 0 through
     //10 or showing the alphabet, which requires a wider view.
     if (self.alphabetViewController.view == self.alphabetViewController.numbersView) {
         self.scrollViewiPhone.contentSize=CGSizeMake(912,50);//912,50
     }
     
     else {
         self.scrollViewiPhone.contentSize=CGSizeMake(2112,50);
     }
...
- (void)viewDidLoad {
    ...
    AlphabetViewController *myAlphabetViewController = [[AlphabetViewController alloc] initWithNibName:@"AlphabetViewController" bundle:nil];
    self.alphabetViewController = myAlphabetViewController;
    
    self.alphabetViewController.chalkboardViewControlleriPhone = self;
    self.scrollViewiPhone.contentSize=CGSizeMake(2112,50);
    [self.scrollViewiPhone addSubview:alphabetViewController.view];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(changeWritingBrushiPhone:)
												 name:@"brushHasBeenSelected" object:nil];
    
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(enableEraseButtoniPhone:)
												 name:@"touchOccuredInView" object:nil];
    
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(disableEraseButtoniPhone:)
												 name:@"chalkboardErasediPhone" object:nil];
...
-(NSInteger)getRecordingsForUpperLowerCategory {
    ...
    if ([array count] >= 1) {
        ...
        for (i = 0; i < countOfLetterRecordingsArray; i++) {
            ...
            NSLog(@"The current letterStrokesCount is %d", totalRecordings);
            
        }
        
    }
    
    NSLog(@"The final letterStrokesCount is %d", totalRecordings);
...
//For some reason, some toolbar buttons do not show they are enabled using the clickedButtonAtIndex method above.
//So, this method has to be used and it works.
    ...
    switch (buttonIndex) {
        case 0:
            ...
            if (guessingGameEnabled == YES) {
                ...
                NSLog(@"the guess game is enabled and supposed to be disabled on button zippy11");
            }
            [chalkboardViewiPhone eraseiPhone];
            self.playStrokesButton.hidden = YES;
            self.playStrokesLabel.hidden = YES;
...
- (void)enableEraseButtoniPhone:(NSNotification *)notification {
	self.eraseButtoniPhone.enabled = YES;
}

- (void)disableEraseButtoniPhone:(NSNotification *)notification {
	self.eraseButtoniPhone.enabled = NO;
}
...
-(void)buttonStyleChanged {
    ...
    self.replayGuessAnimationButtoniPhone.hidden = YES;
    self.replayGuessAnimationLabel.hidden = YES;
    self.guessingGameButtoniPhone.enabled = YES;
    self.cancelGuessGameButtoniPhone.hidden = YES;
...
-(void)changeLetteriPhone:(NSString *)letter forButton:(NSInteger)buttonIdentifier {
        
    orderOfStroke = 0;
    [NSFetchedResultsController deleteCacheWithName:@"Root"];
    self.fetchedResultsController = nil;
    [self.chalkboardViewiPhone eraseiPhone];
    
    //NSLog(@"The center of the big letter is %f, %f", self.letterLabeliPhone.center.x, self.letterLabeliPhone.center.y);
    self.letterLabeliPhone.text = letter;
    self.soundFileForButton = nil;
...
-(void)playbackLetterStroke:(LetterStroke *)letterStroke {
    ...
    CGRect	bounds = self.chalkboardViewiPhone.bounds;
    ...
    for (i = 0; i < myCount - 1; ++i) {
        ...
        [chalkboardViewiPhone drawStrokeFromPoint:firstPoint toPoint:secondPoint];
    }
...
-(IBAction)playAnimationiPhone:(id)sender {
    ...
    NSLog(@"playAnimationiPhone fired");
...
-(void)playUpperLowerAnimation {
	...
	if ([letterLabeliPhone.text isEqualToString:@"Aa"]) {
		...
		AAnimationViewControlleriPhone *aAnimationControlleriPhone = [[AAnimationViewControlleriPhone alloc] initWithNibName:@"AAnimationViewControlleriPhone"
                                                                                                                      bundle:nil];
		...
		[self.view insertSubview:aAnimationControlleriPhone.view atIndex:4];
	...
	if ([letterLabeliPhone.text isEqualToString:@"Bb"]) {
		...
		BAnimationViewControlleriPhone *bAnimationControlleriPhone = [[BAnimationViewControlleriPhone alloc] initWithNibName:@"BAnimationViewControlleriPhone"
																													  bundle:nil];
        ...
        [self.view insertSubview:bAnimationControlleriPhone.view atIndex:4];
	...
	if ([letterLabeliPhone.text isEqualToString:@"Cc"]) {
		...
		CAnimationViewControlleriPhone *cAnimationControlleriPhone = [[CAnimationViewControlleriPhone alloc] initWithNibName:@"CAnimationViewControlleriPhone"
																													  bundle:nil];
        ...
        [self.view insertSubview:cAnimationControlleriPhone.view atIndex:4];
	...
	if ([letterLabeliPhone.text isEqualToString:@"Dd"]) {
		...
		DAnimationViewControlleriPhone *dAnimationControlleriPhone = [[DAnimationViewControlleriPhone alloc] initWithNibName:@"DAnimationViewControlleriPhone"
																													  bundle:nil];
        ...
        [self.view insertSubview:dAnimationControlleriPhone.view atIndex:4];
	...
	if ([letterLabeliPhone.text isEqualToString:@"Ee"]) {
		...
		EAnimationViewControlleriPhone *eAnimationControlleriPhone = [[EAnimationViewControlleriPhone alloc] initWithNibName:@"EAnimationViewControlleriPhone"
																													  bundle:nil];
        ...
        [self.view insertSubview:eAnimationControlleriPhone.view atIndex:4];
	...
	if ([letterLabeliPhone.text isEqualToString:@"Ff"]) {
		...
		FAnimationViewControlleriPhone *fAnimationControlleriPhone = [[FAnimationViewControlleriPhone alloc] initWithNibName:@"FAnimationViewControlleriPhone"
																													  bundle:nil];
        ...
        [self.view insertSubview:fAnimationControlleriPhone.view atIndex:4];
	...
	if ([letterLabeliPhone.text isEqualToString:@"Gg"]) {
		...
		GAnimationViewControlleriPhone *gAnimationControlleriPhone = [[GAnimationViewControlleriPhone alloc] initWithNibName:@"GAnimationViewControlleriPhone"
																													  bundle:nil];
        ...
        [self.view insertSubview:gAnimationControlleriPhone.view atIndex:4];
	...
	if ([letterLabeliPhone.text isEqualToString:@"Hh"]) {
		...
		HAnimationViewControlleriPhone *hAnimationControlleriPhone = [[HAnimationViewControlleriPhone alloc] initWithNibName:@"HAnimationViewControlleriPhone"
																													  bundle:nil];
        ...
        [self.view insertSubview:hAnimationControlleriPhone.view atIndex:4];
	...
	if ([letterLabeliPhone.text isEqualToString:@"Ii"]) {
		...
		IAnimationViewControlleriPhone *iAnimationControlleriPhone = [[IAnimationViewControlleriPhone alloc] initWithNibName:@"IAnimationViewControlleriPhone"
																													  bundle:nil];
        ...
        [self.view insertSubview:iAnimationControlleriPhone.view atIndex:4];
	...
	if ([letterLabeliPhone.text isEqualToString:@"Jj"]) {
		...
		JAnimationViewControlleriPhone *jAnimationControlleriPhone = [[JAnimationViewControlleriPhone alloc] initWithNibName:@"JAnimationViewControlleriPhone"
																													  bundle:nil];
        ...
        [self.view insertSubview:jAnimationControlleriPhone.view atIndex:4];
	...
	if ([letterLabeliPhone.text isEqualToString:@"Kk"]) {
		...
		KAnimationViewControlleriPhone *kAnimationControlleriPhone = [[KAnimationViewControlleriPhone alloc] initWithNibName:@"KAnimationViewControlleriPhone"
																													  bundle:nil];
        ...
        [self.view insertSubview:kAnimationControlleriPhone.view atIndex:4];
	...
	if ([letterLabeliPhone.text isEqualToString:@"Ll"]) {
		...
		LAnimationViewControlleriPhone *lAnimationControlleriPhone = [[LAnimationViewControlleriPhone alloc] initWithNibName:@"LAnimationViewControlleriPhone"
																													  bundle:nil];
        ...
        [self.view insertSubview:lAnimationControlleriPhone.view atIndex:4];
	...
	if ([letterLabeliPhone.text isEqualToString:@"Mm"]) {
		...
		MAnimationViewControlleriPhone *mAnimationControlleriPhone = [[MAnimationViewControlleriPhone alloc] initWithNibName:@"MAnimationViewControlleriPhone"
																													  bundle:nil];
        ...
        [self.view insertSubview:mAnimationControlleriPhone.view atIndex:4];
	...
	if ([letterLabeliPhone.text isEqualToString:@"Nn"]) {
		...
		NAnimationViewControlleriPhone *nAnimationControlleriPhone = [[NAnimationViewControlleriPhone alloc] initWithNibName:@"NAnimationViewControlleriPhone"
																													  bundle:nil];
        ...
        [self.view insertSubview:nAnimationControlleriPhone.view atIndex:4];
	...
	if ([letterLabeliPhone.text isEqualToString:@"Oo"]) {
		...
		OAnimationViewControlleriPhone *oAnimationControlleriPhone = [[OAnimationViewControlleriPhone alloc] initWithNibName:@"OAnimationViewControlleriPhone"
																													  bundle:nil];
        ...
        [self.view insertSubview:oAnimationControlleriPhone.view atIndex:4];
	...
	if ([letterLabeliPhone.text isEqualToString:@"Pp"]) {
		...
		PAnimationViewControlleriPhone *pAnimationControlleriPhone = [[PAnimationViewControlleriPhone alloc] initWithNibName:@"PAnimationViewControlleriPhone"
																													  bundle:nil];
        ...
        [self.view insertSubview:pAnimationControlleriPhone.view atIndex:4];
	...
	if ([letterLabeliPhone.text isEqualToString:@"Qq"]) {
		...
		QAnimationViewControlleriPhone *qAnimationControlleriPhone = [[QAnimationViewControlleriPhone alloc] initWithNibName:@"QAnimationViewControlleriPhone"
																													  bundle:nil];
        ...
        [self.view insertSubview:qAnimationControlleriPhone.view atIndex:4];
	...
	if ([letterLabeliPhone.text isEqualToString:@"Rr"]) {
		...
		RAnimationViewControlleriPhone *rAnimationControlleriPhone = [[RAnimationViewControlleriPhone alloc] initWithNibName:@"RAnimationViewControlleriPhone"
																													  bundle:nil];
        ...
        [self.view insertSubview:rAnimationControlleriPhone.view atIndex:4];
	...
	if ([letterLabeliPhone.text isEqualToString:@"Ss"]) {
		...
		SAnimationViewControlleriPhone *sAnimationControlleriPhone = [[SAnimationViewControlleriPhone alloc] initWithNibName:@"SAnimationViewControlleriPhone"
																													  bundle:nil];
        ...
        [self.view insertSubview:sAnimationControlleriPhone.view atIndex:4];
	...
	if ([letterLabeliPhone.text isEqualToString:@"Tt"]) {
		...
		TAnimationViewControlleriPhone *tAnimationControlleriPhone = [[TAnimationViewControlleriPhone alloc] initWithNibName:@"TAnimationViewControlleriPhone"
																													  bundle:nil];
        ...
        [self.view insertSubview:tAnimationControlleriPhone.view atIndex:4];
	...
	if ([letterLabeliPhone.text isEqualToString:@"Uu"]) {
		...
		UAnimationViewControlleriPhone *uAnimationControlleriPhone = [[UAnimationViewControlleriPhone alloc] initWithNibName:@"UAnimationViewControlleriPhone"
																													  bundle:nil];
        ...
        [self.view insertSubview:uAnimationControlleriPhone.view atIndex:4];
	...
	if ([letterLabeliPhone.text isEqualToString:@"Vv"]) {
		...
		VAnimationViewControlleriPhone *vAnimationControlleriPhone = [[VAnimationViewControlleriPhone alloc] initWithNibName:@"VAnimationViewControlleriPhone"
																													  bundle:nil];
        ...
        [self.view insertSubview:vAnimationControlleriPhone.view atIndex:4];
	...
	if ([letterLabeliPhone.text isEqualToString:@"Ww"]) {
		...
		WAnimationViewControlleriPhone *wAnimationControlleriPhone = [[WAnimationViewControlleriPhone alloc] initWithNibName:@"WAnimationViewControlleriPhone"
																													  bundle:nil];
        ...
        [self.view insertSubview:wAnimationControlleriPhone.view atIndex:4];
	...
	if ([letterLabeliPhone.text isEqualToString:@"Xx"]) {
		...
		XAnimationViewControlleriPhone *xAnimationControlleriPhone = [[XAnimationViewControlleriPhone alloc] initWithNibName:@"XAnimationViewControlleriPhone"
																													  bundle:nil];
        ...
        [self.view insertSubview:xAnimationControlleriPhone.view atIndex:4];
	...
	if ([letterLabeliPhone.text isEqualToString:@"Yy"]) {
		...
		YAnimationViewControlleriPhone *yAnimationControlleriPhone = [[YAnimationViewControlleriPhone alloc] initWithNibName:@"YAnimationViewControlleriPhone"
																													  bundle:nil];
        ...
        [self.view insertSubview:yAnimationControlleriPhone.view atIndex:4];
	...
	if ([letterLabeliPhone.text isEqualToString:@"Zz"]) {
		...
		ZAnimationViewControlleriPhone *zAnimationControlleriPhone = [[ZAnimationViewControlleriPhone alloc] initWithNibName:@"ZAnimationViewControlleriPhone"
																													  bundle:nil];
        ...
        [self.view insertSubview:zAnimationControlleriPhone.view atIndex:4];
...
-(void)playNumberAnimation {
	...
	if ([letterLabeliPhone.text isEqualToString:@"0"]) {
		...
		ZeroAnimationViewControlleriPhone *zeroAnimationControlleriPhone = [[ZeroAnimationViewControlleriPhone alloc] initWithNibName:@"ZeroAnimationViewControlleriPhone"
                                                                                                                      bundle:nil];
		...
		[self.view insertSubview:zeroAnimationViewControlleriPhone.view atIndex:4];
	...
	if ([letterLabeliPhone.text isEqualToString:@"1"]) {
		...
		OneAnimationViewControlleriPhone *oneAnimationControlleriPhone = [[OneAnimationViewControlleriPhone alloc] initWithNibName:@"OneAnimationViewControlleriPhone"
                                                                                                                               bundle:nil];
		...
		[self.view insertSubview:oneAnimationViewControlleriPhone.view atIndex:4];
    ...
    if ([letterLabeliPhone.text isEqualToString:@"2"]) {
		...
		TwoAnimationViewControlleriPhone *twoAnimationControlleriPhone = [[TwoAnimationViewControlleriPhone alloc] initWithNibName:@"TwoAnimationViewControlleriPhone"
                                                                                                                            bundle:nil];
		...
		[self.view insertSubview:twoAnimationViewControlleriPhone.view atIndex:4];
    ...
    if ([letterLabeliPhone.text isEqualToString:@"3"]) {
		...
		ThreeAnimationViewControlleriPhone *threeAnimationControlleriPhone = [[ThreeAnimationViewControlleriPhone alloc] initWithNibName:@"ThreeAnimationViewControlleriPhone"
                                                                                                                   bundle:nil];
		...
		[self.view insertSubview:threeAnimationViewControlleriPhone.view atIndex:4];
    ...
    if ([letterLabeliPhone.text isEqualToString:@"4"]) {
		...
		FourAnimationViewControlleriPhone *fourAnimationControlleriPhone = [[FourAnimationViewControlleriPhone alloc] initWithNibName:@"FourAnimationViewControlleriPhone"
                                                                                                                   bundle:nil];
		...
		[self.view insertSubview:fourAnimationViewControlleriPhone.view atIndex:4];
    ...
    if ([letterLabeliPhone.text isEqualToString:@"5"]) {
		...
		FiveAnimationViewControlleriPhone *fiveAnimationControlleriPhone = [[FiveAnimationViewControlleriPhone alloc] initWithNibName:@"FiveAnimationViewControlleriPhone"
                                                                                                                   bundle:nil];
		...
		[self.view insertSubview:fiveAnimationViewControlleriPhone.view atIndex:4];
    ...
    if ([letterLabeliPhone.text isEqualToString:@"6"]) {
		...
		SixAnimationViewControlleriPhone *sixAnimationControlleriPhone = [[SixAnimationViewControlleriPhone alloc] initWithNibName:@"SixAnimationViewControlleriPhone"
                                                                                                                   bundle:nil];
		...
		[self.view insertSubview:sixAnimationViewControlleriPhone.view atIndex:4];
    ...
    if ([letterLabeliPhone.text isEqualToString:@"7"]) {
		...
		SevenAnimationViewControlleriPhone *sevenAnimationControlleriPhone = [[SevenAnimationViewControlleriPhone alloc] initWithNibName:@"SevenAnimationViewControlleriPhone"
                                                                                                                   bundle:nil];
		...
		[self.view insertSubview:sevenAnimationViewControlleriPhone.view atIndex:4];
    ...
    if ([letterLabeliPhone.text isEqualToString:@"8"]) {
		...
		EightAnimationViewControlleriPhone *eightAnimationControlleriPhone = [[EightAnimationViewControlleriPhone alloc] initWithNibName:@"EightAnimationViewControlleriPhone"
                                                                                                                   bundle:nil];
		...
		[self.view insertSubview:eightAnimationViewControlleriPhone.view atIndex:4];
    ...
    if ([letterLabeliPhone.text isEqualToString:@"9"]) {
		...
		NineAnimationViewControlleriPhone *nineAnimationControlleriPhone = [[NineAnimationViewControlleriPhone alloc] initWithNibName:@"NineAnimationViewControlleriPhone"
                                                                                                                   bundle:nil];
		...
		[self.view insertSubview:nineAnimationViewControlleriPhone.view atIndex:4];
    ...
    if ([letterLabeliPhone.text isEqualToString:@"10"]) {
		...
		TenAnimationViewControlleriPhone *tenAnimationControlleriPhone = [[TenAnimationViewControlleriPhone alloc] initWithNibName:@"TenAnimationViewControlleriPhone"
                                                                                                                   bundle:nil];
		...
		[self.view insertSubview:tenAnimationViewControlleriPhone.view atIndex:4];
...
-(void)sayLetteriPhone {
    ...
    if ([letterLabeliPhone.text hasSuffix:@"c"] || [letterLabeliPhone.text hasPrefix:@"C"]) {
        NSLog(@"Yes, the button has the suffix a or prefix C");
...
-(IBAction)guessGameButtonPressed:(id)sender {
    ...
    NSLog(@"guessGameButtonPressed fired.");
    
    [self.chalkboardViewiPhone eraseiPhone];
    self.cancelGuessGameButtoniPhone.hidden = NO;
...
-(void)playUpperLowerRandomAnimation {
    ...
    [self startGuessGameAudioiPhone];

	guessingGameEnabled = YES;
	self.letterLabeliPhone.text = @"";
    self.playAnimationsButton.enabled = NO;
	self.guessingGameButtoniPhone.enabled = NO;
	self.replayGuessAnimationButtoniPhone.hidden = NO;
...
-(IBAction)replayGuessAnimationiPhone:(id)sender {
    ...
    NSLog(@"replayGuessAnimationiPhone fired.");
...
//This method chooses a random audio file to play when the guess letter game is started.
-(void)playGuessLetterAudio {
    NSLog(@"playGuessLetterAudio fired");
...
-(void)rightAnswerAudioiPhone {
    ...
    NSLog(@"rightAnswerAudioiPhone fired.");
...
-(void)wrongAnswerAudioiPhone {
    ...
    NSLog(@"wrongAnswerAudioiPhone fired.");
...
-(void)changeWritingBrushiPhone:(NSNotification *)notification {
	...
	[self.chalkboardViewiPhone changeBrushiPhone:nil];
}
...
-(IBAction)changeAnimationSpeediPhone:(id)sender {
    ...
    NSLog(@"changeAnimationSpeediPhone fired in ChalkboardViewControlleriPhone with value of %f", animationSpeediPhone);
...
- (void)stopAnimationsiPhone {
	...
	NSLog(@"stopAnimationsiPhone fired.");
...
- (BOOL)shouldAutorotateToInterfaceOrientation:(UIInterfaceOrientation)interfaceOrientation {
    ...
    //return (interfaceOrientation == (UIInterfaceOrientationPLandscapeLeft | UIInterfaceOrientationPorLandscapeRight));
    return YES;
}

-(void)willAnimateRotationToInterfaceOrientation:(UIInterfaceOrientation)toInterfaceOrientation duration:(NSTimeInterval)duration {
    
    NSLog(@"willAnimateRotation just fired in ChalkboardViewControlleriPhone");
...
- (void)didReceiveMemoryWarning {
    ...
    NSLog(@"didReceiveMemoryWarning fired in chalkboardViewControlleriPhone");
...
- (void)viewDidUnload {
    ...
    NSLog(@"viewDidUnload fired in ChalkboardViewControlleriPhone");
...
//	self.letterLabeliPhone = nil;
	...
	self.replayGuessAnimationButtoniPhone = nil;
	self.guessingGameButtoniPhone = nil;
	self.eraseButtoniPhone = nil;
	//self.maximizeScreenButtoniPhone = nil;
	self.settingsMenuButtoniPhone = nil;
