...
@interface ChalkboardViewiPhone (private)
...
static UIImage *myBrushImageiPhone = nil;

@implementation ChalkboardViewiPhone

@synthesize  location;
@synthesize  previousLocation;
@synthesize myAnimationViewiPhone;
@synthesize chalkboardViewControlleriPhone;
@synthesize delegate;

//declare static variable methods - jh
+ (UIImage*) getMyBrushImageiPhone {
	return myBrushImageiPhone;
}

+ (void) setMyBrushImageiPhone:(UIImage*)newMyBrushImageiPhone {
    if(myBrushImageiPhone != newMyBrushImageiPhone) {
		myBrushImageiPhone = newMyBrushImageiPhone;
	}
}
...
+ (void)initialize {
	if(! myBrushImageiPhone ) {
		myBrushImageiPhone = [UIImage imageNamed:@"5Fingerprint.png"];
	}
}
...
- (id)initWithCoder:(NSCoder*)coder {
    ...
    if ((self = [super initWithCoder:coder])) {
        ...
        if ([UIScreen mainScreen].scale > 1.0) {
            myBrushScale = 2.0;
            //myBrushPixelStep = .50; Same for whether it's Retina or not.
        }
		...
		//brushImageiPhone = [UIImage imageNamed:@"22WhiteFlat.png"].CGImage;
		
		NSLog(@"The value of brushImage in ChalkboardView before the getMyBrushImage is %@", brushImageiPhone);
		//use static variable - jh
		brushImageiPhone = [ChalkboardViewiPhone getMyBrushImageiPhone].CGImage;
		NSLog(@"The value of brushImage in ChalkboardView after the getMyBrushImage is %@", brushImageiPhone);
        
		
		// Get the width and height of the image
		width = (CGImageGetWidth(brushImageiPhone))*.25;
		height = (CGImageGetHeight(brushImageiPhone))*.25;
		...
		if(brushImageiPhone) {
			...
			brushContext = CGBitmapContextCreate(brushData, width, width, 8, width * 4, CGImageGetColorSpace(brushImageiPhone), kCGImageAlphaPremultipliedLast);
			// After you create the context, you can draw the  image to the context.
			CGContextDrawImage(brushContext, CGRectMake(0.0, 0.0, (CGFloat)width, (CGFloat)height), brushImageiPhone);
...
- (void)eraseiPhone {
	...
	[[NSNotificationCenter defaultCenter] postNotificationName:@"chalkboardErasediPhone" object:self];

}

-(IBAction)eraseChalkboardiPhone:(id)sender {
	
	NSLog(@"eraseChalkboardiPhone in ChalkboardViewiPhone just fired");
...
-(void)layoutSubviews {
	...
	[self eraseChalkboardiPhone:nil];
	...
	NSLog(@"layoutSubviews in chalkboardViewiPhone just fired");


/*
//FIX THIS FOR v4.0
    //These methods make the paper lines image appear in the right spot after the switch is switched
    //in the settings view, and then the device is rotated and then the back button is pushed.
    if ([[UIDevice currentDevice] orientation] == UIInterfaceOrientationLandscapeLeft ||
        [[UIDevice currentDevice] orientation] == UIInterfaceOrientationLandscapeRight) {
        
        self.chalkboardViewControlleriPhone.guidlinesImage.center = CGPointMake(240, 172.500);
    }
    
    else {
        
        self.chalkboardViewControlleriPhone.guidlinesImage.center = CGPointMake(160, 247.500);
    }
*/
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
	...
	//code i found on web to record touch coordinates and print to console - j
	//touchCoordinates.x and touchCoordinates.y give you information you need.
	CGPoint troubleshootCoordinates = [touch locationInView:myAnimationViewiPhone];
	NSLog(@"touchesBegan at = %f,%f", troubleshootCoordinates.x, troubleshootCoordinates.y);
